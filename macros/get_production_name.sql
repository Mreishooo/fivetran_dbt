{% macro get_production_name( country,  page_path ) %}
   CASE
      ...
      WHEN ( LOWER(  {{ page_path }}) LIKE '%mamma%') THEN 'DE - MM - TNF - 1'
      ...
      WHEN ( LOWER(  {{ page_path }}) LIKE '%wicked%') THEN 'DE - WI - TNF - 1'
      WHEN ( LOWER(  {{ page_path }}) LIKE '%tina%'AND LOWER(  {{ page_path }}) LIKE '%hamburg%' ) THEN 'DE - TIN - OPH - 1'
      WHEN ( LOWER(  {{ page_path }}) LIKE '%tina%' AND LOWER(  {{ page_path }}) LIKE '%stuttgart%' ) THEN 'DE - TIN - APO - 1'
      WHEN ( LOWER(  {{ page_path }}) LIKE '%/tina%' ) THEN 'Tina'
      WHEN ( LOWER(  {{ page_path }}) LIKE '%blue%') THEN 'DE - BMG - BMX - 1'
      WHEN ( LOWER(  {{ page_path }}) LIKE '%kudamm%'
      	OR LOWER(  {{ page_path }}) LIKE "%ku\'damm%" ) THEN 'DE - KD - TDW - 1'
      WHEN ( LOWER(  {{ page_path }}) LIKE '%aladdin%') THEN 'DE - ALA - APO - 1'
      WHEN ( LOWER(  {{ page_path }}) LIKE '%vampire%') THEN 'DE - TDV - PAL - 3'
